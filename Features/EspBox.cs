...
public static class EspBox
{
    ...
    public static void Draw(Graphics.Graphics graphics)
    {
        ...
        {
            var colorBox = entity.Team == Team.Terrorists ? Color.DarkRed : Color.DarkBlue; // change color add config var
        ...
        (Vector2, Vector2) boundingBox)
    {
        ...
        // Create a copy of the BonePos collection to avoid modifying it while iterating


        var healthPercentage = (float)entity.Health / 100;

        if (entity.Team == graphics.GameData.Player.Team && config.config.b_t_Box)
        {
        ...
        {
            if (entity.Team != graphics.GameData.Player.Team && config.config.b_e_Box)
            {
                graphics.DrawRectangle(color, boundingBox.Item1, boundingBox.Item2);
            }
        ...
        if (entity.Team == graphics.GameData.Player.Team && config.config.b_t_Healthbar)
        {
            DrawHealthBar(graphics, healthBarTopLeft, healthBarBottomRight, healthPercentage);
        }
        else
        {
            if (entity.Team != graphics.GameData.Player.Team && config.config.b_e_Healthbar)
            {
                DrawHealthBar(graphics, healthBarTopLeft, healthBarBottomRight, healthPercentage);
            }
        }

        if (entity.Team == graphics.GameData.Player.Team && config.config.b_t_Healthnum)
        {
            DrawHealthNumber(graphics, boundingBox.Item1, boundingBox.Item2, entity.Health);
        }
        else
        {
            if (entity.Team != graphics.GameData.Player.Team && config.config.b_e_Healthnum)
            {
                DrawHealthNumber(graphics, boundingBox.Item1, boundingBox.Item2, entity.Health);
            }
        ...
        if (entity.Team == graphics.GameData.Player.Team && config.config.b_t_weaponName)
        {
            DrawWeaponName(graphics, boundingBox, entity.CurrentWeaponName);
        }
        else
        {
            if (entity.Team != graphics.GameData.Player.Team && config.config.b_e_weaponName)
            {
                DrawWeaponName(graphics, boundingBox, entity.CurrentWeaponName);
        ...
        if (entity.Team == graphics.GameData.Player.Team && config.config.b_t_playerName)
        {
            DrawPlayerName(graphics, boundingBox, entity.Name);

        }
        else
        {
            if (entity.Team != graphics.GameData.Player.Team && config.config.b_e_playerName)
            {
                DrawPlayerName(graphics, boundingBox, entity.Name);
        ...
        //DrawFlags(graphics, boundingBox, entity); //Flags questionable, awaiting implementation
    }
    ...
    private static void DrawPlayerName(Graphics.Graphics graphics, (Vector2, Vector2) boundingBox, string enemyName)
    {
    ...
    private static void DrawFlags(Graphics.Graphics graphics, (Vector2, Vector2) boundingBox, Entity entity)
    {
        ...
        if (entity.IsSpotted)
        {
            graphics.FontConsolas32.DrawText(default, "HIT", (int)flagsPosition.X, (int)flagsPosition.Y,
            Color.Red);
    ...
    private static (Vector2, Vector2) GetEntityBoundingBox(Graphics.Graphics graphics, Entity entity)
    {
        ...
        // Create a copy of the BonePos collection
        var bonePosList = entity.BonePos.Values.ToList();

        foreach (var transformedPos in bonePosList
                     .Select(bonePos => graphics.GameData.Player.MatrixViewProjectionViewport.Transform(bonePos))
