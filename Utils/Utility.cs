...
public static class Utility
{
    ...
    public static void Write<T>(this System.Diagnostics.Process process, IntPtr lpBaseAddress, T value)
    where T : unmanaged
    {
        Write(process.Handle, lpBaseAddress, value);
    }

    public static void Write<T>(this Module module, int offset, T value)
        where T : unmanaged
    {
        Write(module.Process.Handle, module.ProcessModule.BaseAddress + offset, value);
    }

    private static void Write<T>(IntPtr hProcess, IntPtr lpBaseAddress, T value)
        where T : unmanaged
    {
        var size = Marshal.SizeOf<T>();
        ...
        try
        {
            ...
            Kernel32_.WriteProcessMemory(hProcess, lpBaseAddress, buffer, (uint)size, out _);
        }
        finally
        {
    ...
    private static void WriteBytes(IntPtr hProcess, IntPtr lpBaseAddress, byte[] bytes)
    {
        Kernel32_.WriteProcessMemory(hProcess, lpBaseAddress, bytes, (uint)bytes.Length, out _);
    }


    public static void WriteString(this System.Diagnostics.Process process, IntPtr lpBaseAddress, string value)
    {
        var bytes = Encoding.UTF8.GetBytes(value + '\0');
        WriteBytes(process.Handle, lpBaseAddress, bytes);
    }
